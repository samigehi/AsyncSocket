...
public class ByteBufferReader {
    private ArrayDeque<ByteBuffer> mBuffers = new ArrayDeque<ByteBuffer>();
    ...
    public ByteBufferReader(byte[] buf) {
        super();
    ...
    public ByteBufferReader addAll(ByteBufferReader... bb) {
        for (ByteBufferReader b: bb)
            b.get(this);
        return this;
    }
    ...
    public void get(ByteBufferReader into, int length) {
        if (remaining() < length)
            throw new IllegalArgumentException("length");
        int offset = 0;
    ...
    public void get(ByteBufferReader into) {
        get(into, remaining());
    }

    public ByteBufferReader get(int length) {
        ByteBufferReader ret = new ByteBufferReader();
        get(ret, length);
    ...
    public ByteBufferReader add(ByteBufferReader b) {
        b.get(this);
        return this;
    }

    public ByteBufferReader add(ByteBuffer b) {
        if (b.remaining() <= 0) {
//            System.out.println("reclaiming remaining: " + b.remaining());
    ...
    public void recycle() {
        ...
        //if (mBuffers.size() != 0) throw new AssertionError();
        remaining = 0;
    }
...
//                System.out.println("too full: " + b.capacity());
            ...
            r.add(b);
            if ((r.size() != 0) == (currentSize == 0)) return;
